...
function imageMatch(i){

  for (let i=10; i < nodes.length; i++) {
    for (let j=0; j < prediction_data.length; j++) {
      if (String(nodes[i].prob/10) == prediction_data.predictions[j].probability) {
        return prediction_data.predictions[j].image.src;
      };
    };
  };
};
...
for (let x = 0; x < 10; x++) {
  for (let i = 0; i < top_ten_class_probas[top_ten_freq[x][0]].length; i++) {
    nodes.push({"name": "Class"+" "+x+" "+"image"+i, "freq": (top_ten_class_probas[top_ten_freq[x][0]][i])*10, "child": true, "src":""})//
  };
};
